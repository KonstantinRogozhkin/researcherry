...
build_darwin_main() { # {{{
  if [[ ! -f "${SRC_PREFIX}src/${QUALITY}/resources/darwin/code.icns" ]]; then
    rsvg-convert -w 655 -h 655 "${QUALITY}/re-high-resolution-logo-transparent.svg" -o "code_logo.png"
    composite "code_logo.png" -gravity center "template_macos.png" "code_1024.png"
...
build_darwin_types() { # {{{
  rsvg-convert -w 128 -h 128 "${QUALITY}/re-high-resolution-logo-transparent.svg" -o "code_logo.png"
...
build_linux_main() { # {{{
  if [[ ! -f "${SRC_PREFIX}src/${QUALITY}/resources/linux/code.png" ]]; then
    rsvg-convert -w 512 -h 512 "${QUALITY}/re-high-resolution-logo-transparent.svg" -o "${SRC_PREFIX}src/${QUALITY}/resources/linux/code.png"
  fi
...
build_media() { # {{{
  if [[ ! -f "${SRC_PREFIX}src/${QUALITY}/src/vs/workbench/browser/media/code-icon.svg" ]]; then
    cp "${QUALITY}/re-high-resolution-logo-transparent.svg" "${SRC_PREFIX}src/${QUALITY}/src/vs/workbench/browser/media/code-icon.svg"
    gsed -i 's|width="[0-9]*" height="[0-9]*"|width="1024" height="1024"|' "${SRC_PREFIX}src/${QUALITY}/src/vs/workbench/browser/media/code-icon.svg"
  fi
} # }}}

build_server() { # {{{
  if [[ ! -f "${SRC_PREFIX}src/${QUALITY}/resources/server/favicon.ico" ]]; then
    rsvg-convert -w 256 -h 256 "${QUALITY}/re-high-resolution-logo-transparent.svg" -o "temp_favicon.png"
    convert "temp_favicon.png" -define icon:auto-resize=256,128,96,64,48,32,24,16 "${SRC_PREFIX}src/${QUALITY}/resources/server/favicon.ico"
    rm "temp_favicon.png"
  fi
...
build_windows_main() { # {{{
  if [[ ! -f "${SRC_PREFIX}src/${QUALITY}/resources/win32/code.ico" ]]; then
    rsvg-convert -w 256 -h 256 "${QUALITY}/re-high-resolution-logo-transparent.svg" -o "temp_icon.png"
    convert "temp_icon.png" -define icon:auto-resize=256,128,96,64,48,32,24,16 "${SRC_PREFIX}src/${QUALITY}/resources/win32/code.ico"
    rm "temp_icon.png"
  fi
} # }}}
...
build_windows_type() { # {{{
  ...
  if [[ ! -f "${FILE_PATH}" ]]; then
    ...
    rsvg-convert -w "${LOGO_SIZE}" -h "${LOGO_SIZE}" "${QUALITY}/re-high-resolution-logo-transparent.svg" -o "code_logo.png"
...
build_windows_types() { # {{{
  ...
  rsvg-convert -b "#F5F6F7" -w 64 -h 64 "${QUALITY}/re-high-resolution-logo-transparent.svg" -o "code_logo.png"
